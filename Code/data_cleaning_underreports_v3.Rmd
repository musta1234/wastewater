...
preq = c( "dplyr", "ggplot2", "ggfortify", "readxl", "readr", 
          "stats", "tidyverse", "haven", "Matrix", "foreign", "zoo")
#for (y in preq) install.packages(y, dep = TRUE)
sapply(preq, library, character.only=T)

# Set the directory where the files are located
setwd("C:/Users/mustam23/OneDrive\ -\ Pfizer/Documents/Projects/underreporting")
...
# Create a column for the 7 day moving average cases/inf by location using zoo::rollapplyr
  ...
  select(location_name, date, inf_mean, daily_cases, daily_cases_7d,
         admis_mean, all_bed_capacity, cases_mean, cumulative_all_effectively_vaccinated,
         cumulative_all_fully_vaccinated, cumulative_all_vaccinated, cumulative_cases, cumulative_deaths,
         cumulative_deaths_unscaled, cumulative_hospitalizations, daily_deaths, daily_deaths_unscaled,
         hospital_beds_mean, icu_bed_capacity, icu_beds_mean, inf_cuml_mean, infection_detection,
         infection_fatality, infection_hospitalization, mandates_mean, mask_use_mean, mask_use_obs,
         pneumonia_mean, pneumonia_obs, population) %>%
# select(location_name, date, inf_mean, daily_cases, daily_cases_7d) %>%
...
# Calculate case ascertainment as reported cases (7day avg / inf_mean) after discussion with Hannah
  ...
  mutate(case_detection = daily_cases_7d/inf_mean) %>% 
...
summarize_ihme <- function(ihme_data, var1, var2 = ""){
  ihme_data %>%
    ...
    group_by({{var1}}, {{var2}}) %>% 
    summarize(
      n_ascert = sum(!is.na(case_detection)),
...
write.csv(data_regions, 
             file = paste0(getwd(), "/output/" ,"bigfile", ".csv"),
             quote = TRUE,
             na = "",
             row.names = FALSE)
