...
ww_aus_import <- 
  read.csv(paste0(data_loc, "/", ww_aus), sep = ";") %>% View()
...
hosp_aus_import2 <-  
  ...
  mutate(predate = as.Date(paste0(Year, "-01-01")) + 7 * as.numeric(Week) - 1,
         new_week = ifelse(epiweek(predate) == as.numeric(Week), 
                           # if epiweek is the same as the week number then we are good else we need to adjust the week number to give the right epiweek
                            as.numeric(Week),
                            epiweek(predate)),
...
cases_can_import <- 
  ...
  select(prname, date, numtotal_last7, numdeaths_last7, ratecases_last7, 
         ratedeaths_last7) %>%
  #cases_7dts = Number of cases reported in the reporting week
  #deaths_7dts = Number of deaths reported in the reporting week
  #cases_7dts_rate = Rate of cases reported in the reporting week
  #deaths_7dts_rate = Rate of deaths reported in the reporting week
...
ww_can_import <- 
  ...
  group_by(Date) %>%
  summarize(ww_load_median = median(viral_load),
            ww_load_mean = mean(viral_load)
            ) %>%
