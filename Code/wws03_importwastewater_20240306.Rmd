...
correction_factor2 <- function(year) {
  #force year to be an integer; # is this corrections factor related to weekdays(date)
...
hosp_fin_import <- #hosp_new is new hospitalizations which is not found in OWID data
  ...
  mutate(location_name = "Finland",
         ...
         weekday_hosp=weekdays(date),
         next_saturday = next_saturday(date)
         #epi_week = Week, #year = Year
         )
...
ww_neth_import <- 
  ...
  mutate(location_name = "Netherlands",
         weekday = weekdays(date),
         next_saturday = next_saturday(date)) %>%
  group_by(next_saturday) %>%
  mutate(viral_load_median_wkly = mean(viral_load_median, na.rm = TRUE),
         viral_load_mean_wkly = mean(viral_load_mean, na.rm = TRUE),
         count_wkly = mean(count, na.rm = TRUE)
         )
...
hosp_neth_import <- 
  ...
  filter(location_name == "Netherlands" & 
           grepl("Weekly", indicator) &
           !grepl("million", indicator)
         ) %>%
  mutate(next_saturday = next_saturday(date)) %>%
  group_by(next_saturday) %>%
  mutate(hosp_new_wkly = mean(as.numeric(value), na.rm = TRUE))
...
ww_fin_hosp <-
  full_join(ww_fin_import, hosp_fin_import, by = c("next_saturday", "location_name")) %>%
  arrange(., desc(next_saturday)) %>%
  rename(
    date_ww = date.x, date_hosp = date.y,
  )
